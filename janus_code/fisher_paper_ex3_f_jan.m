%fisher_paper_ex1_f.m written 10-22-16 by JTN to simulate
%u_t + (f(s(t))g(m)u)_m = D(m)*u_xx + lambda(m)*u(1-w) for a given value of
%alpha

%Example 1 : Threshold at m = 1 : f(s) = 1 , g(m) = m(1-m)

function z = fisher_paper_ex3_f_jan(alpha,beta,gamma,t,x,m,X,M,IC)


    %Construct vectors of independent variables
    mn = length(m); %number of m points
    xn = length(x); %number of x points
    tn = length(t);
    total = mn*xn;
